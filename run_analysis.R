...
activity_labels <- read.table("./data/project/activity_labels.txt")[,2]

# Load data column 
feature <- read.table("./data/project/features.txt")[,2]

# Extract the measurements on the mean and standard deviation for each measurement.
feature2 <- grepl("mean|std", feature)

# Load and process X_test & y_test data.
X_test <- read.table("./data/project/test/X_test.txt")
...
X_train <- read.table("./data/project/train/X_train.txt")
y_train <- read.table("./data/project/train/y_train.txt")

subject_train <- read.table("./data/project/train/subject_train.txt")
